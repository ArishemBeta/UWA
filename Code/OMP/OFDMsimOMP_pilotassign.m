...
for nblk=1: 1
    ...
    pilot_symbol=zeros(size(sc_idx));
    for nt=1:Nt
        pilot_symbol(nt,:)=block_symbol(nt,sc_idx(nt,:).');
    end
...
if(ifOMP)
    ...
    for niter=1:2
        Bar=zeros(Nt,2);% (:,1)data    (:,2)pilot
        Var=zeros(Nt,2);
        for nt=1:Nt
            if(niter==1)
                Bar(nt,1)=0;Var(nt,1)=1;
                Bar(nt,2)=mean(pilot_symbol(nt,:));Var(nt,2)=var(pilot_symbol(nt,:));
            else
                Bar(nt,1)=mean(SO_data(nt,:));Var(nt,1)=var(SO_data(nt,:));
                Bar(nt,2)=mean(pilot_symbol(nt,:));Var(nt,2)=var(pilot_symbol(nt,:));
            end
        end

        tic
%         S_EstO=MIMO_LMMSE_Equalization(z,H,K,pattern.',Nr,Nt,MMode,nKr,N0,Bar,Var);
        S_EstO=MIMO_SBL_equ(z,H,10,1e-6,N0,pattern.',2,K,Var);
        toc
        SO=zeros(K,Nt);
        SO_data=zeros(K,Nt);
        for nt=1:Nt
