...
% load P_bitQPSK.mat;
data_seq=reshape(Data_bit,Nbit_d,length(Data_bit)/(Nbit_d));
pilot_seq=reshape(Pilot_bit,Nbit_p,length(Pilot_bit)/(Nbit_p));
...
%------------频域插零----------
[Tx_sym_ins, Tx_sym]=Txsymbol_arrangement(data_sym_t,pilot_sym_t,K,pattern.',Ns,Nt);
...
for nblk=1: 1
    ...
    block_bit=data_seq(:,nblk:Nblock:end).';
...
%     S_EstO=((H'*H+N0*eye(K))\H'*z).';
    ...
    toc
    SO=(diag(repmat(pattern,1,K/lp))*S_EstO.').';
    SO(SO==0)=[];
    SO=reshape(SO,Nt,[]);
    scatterplot(SO(1,:));
