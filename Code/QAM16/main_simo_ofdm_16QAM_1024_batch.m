...
load One_Tx_Ant1_OFDM_16QAM_K1024.mat  %load Tx symbols
...
%         plot(abs(RX_data_nn(1,172704:254049)));
%         RX_data_nn(i,:)=interp1((0:length(RX_data_n(i,:))-1),RX_data_n(i,:),(0:length(RX_data_n(i,:))-1)/(1-0.0013844),'spline');
    end
    N_shift_symbol= 10;
    N_shift_sample= 0;
    ...
    for nblk= [1]%: 12]%Nrepeat]
        ...
        nblk
        d0=-0.0014;
        Noffset= (1300*Nt+511+189+12*2*1354)/(1+d0); %start of the nblk-th block +(nblk-1)*1354
...
%             Gap=Gap-fix(Ns*(2000+2*12*1354)*(d0));
%             RX_block=RX_data_nn(:,Gap:Gap+K*Ns+(L)*Ns-1);            
%             [doppler_scale(nblk),cfo(nblk),shift(nblk)]=D3Iteration(-0.0025,-0.00005,16,22,1,1,1,sc_idx,Nt,Nr,Ns,K,L,RX_data_nn,Gap,nblk,doppler_scale,pilot_symbol,block_symbol,pilot_bit,block_bit);
            doppler_scale(nblk)=-0.001292;
            cfo(nblk)=19;
            if(nblk>1)
                Gap=Gap+shift(nblk);
                for i=2:nblk
                    Gap= Gap+fix(Ns*1354/(1+doppler_scale(i-1)));%-6770
                end
                RX_block=RX_data_nn(:,Gap:fix(Gap+(K*Ns+(L)*Ns)/(1+doppler_scale(nblk-1)))-1);%
            else
                Gap=Gap+shift(nblk);
                RX_block=RX_data_nn(:,Gap:Gap+K*Ns+(L)*Ns-1);
%                 plot(abs(RX_data_nn(1,Gap:end)));
            end
%             plot(abs(RX_data_nn(1,Gap-1050:Gap+5720)));
