...
%             Y= RX_data(Chnn_idx,Gap: Ns: Gap+K*Ns-1);
            ...
            [doppler_scale(nblk),shift(nblk)]=mseqcorrelate_d_s(-0.003,0,1,1,Nt,Nr,Ns,K,L,Kg,RX_data,nblk,doppler_scale,pilot);
...
%             shift=[0 -10 -20 -30 -40 -40 -30 -20 -10 0 -10 -20];
            Gap=Gap-fix(Ns*2000*doppler_scale(1))+shift(nblk);%
            if(nblk>1)
                for i=1:nblk
                    Gap= Gap-fix(Ns*doppler_scale(nblk-1)*1354);
                end
