function [doppler_scale,cfo]=D2SearchQPSK(ald,ahd,alc,ahc,P,u,v,sc_idx,Nt,Nr,Ns,K,L,RX_block,pilot_symbol,block_symbol,SNR,SNRdB,Nbps)
recx=zeros(2,300);
d=[ald:0.0001:ahd];
c=[alc:0.1:ahc];
% c=alc;(ahc-alc)/2
...
%         recx(1,i)=ad;
%         recx(2,j)=ac;
        BER_cost=BER_cost_d_calculation(RX_block,0,ad,0,ac,Nt,Nr,block_symbol,Ns,K,L,sc_idx,pilot_symbol,SNR,SNRdB,Nbps);
        recy(i,j)=BER_cost;
...
function BER_cost_d=BER_cost_d_calculation(RX_block,deltad,ad,deltac,ac,Nt,Nr,block_symbol,Ns,K,L,sc_idx,pilot_symbol,SNR,SNRdB,Nbps)
for i=1:Nr
