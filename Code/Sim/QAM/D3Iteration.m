function [doppler_scalee,cfoe,shifte]=D3Iteration(ald,ahd,alc,ahc,P,u,v,sc_idx,Nt,Nr,Ns,K,L,RX_data,nblk,doppler_scale,pilot_symbol,block_symbol,SNRdB)
...
BER_cost=BER_cost_d_calculation3(RX_block,0,normld,0,normlc,Nt,Nr,block_symbol,Ns,K,L,sc_idx,pilot_symbol,nad,nbd,nac,nbc,SNRdB);
...
while(1)
    ...
    normlc=rec(2,nnnn);
    BER_cost=rec(4,nnnn);
    BER_cost_dd=BER_cost_d_calculation3(RX_block,deltad,normld,0,normlc,Nt,Nr,block_symbol,Ns,K,L,sc_idx,pilot_symbol,nad,nbd,nac,nbc,SNRdB);
    grad(1,nnnn)=2*sqrt(BER_cost)*(sqrt(BER_cost_dd)-sqrt(BER_cost))/(deltad);%*nad
    BER_cost_dc=BER_cost_d_calculation3(RX_block,0,normld,deltac,normlc,Nt,Nr,block_symbol,Ns,K,L,sc_idx,pilot_symbol,nad,nbd,nac,nbc,SNRdB);
...
%         end
        ...
        kkkkd=kkkkd+1;
        delta_lastd=deltad;
        deltad=max(deltad*(0.1),10^(-P));%+0.01*rand()
...
%         end
        ...
        kkkkc=kkkkc+1;
        delta_lastc=deltac;
        deltac=max(deltac*(0.1),10^(-P));%+0.01*rand()
...
%     if(kkkkd>Pd && kkkkc>Pc)break;end
    if((jumpd+jumpc)~=0)
        ...
        BER_cost=BER_cost_d_calculation3(RX_block,0,normld,0,normlc,Nt,Nr,block_symbol,Ns,K,L,sc_idx,pilot_symbol,nad,nbd,nac,nbc,SNRdB);
%         plotsss(RX_block,0,normld,0,normlc,pilot_bit,Nt,Nr,block_symbol,Ns,K,L,sc_idx,block_bit,pilot_symbol,nad,nbd,nac,nbc);
        if(normld>normhd || normlc>normhc)break;end
        nnnn=nnnn+1;suc=suc+1;
%         if(suc>=3)deltad=1.2*deltad;deltac=1.2*deltac;else if(suc>=2)deltad=0.05+deltad;deltac=0.05+deltac;end;end
        rec(1,nnnn)=normld;
        rec(2,nnnn)=normlc;
        rec(4,nnnn)=BER_cost;
...
shifte=0;
% figure();
% plot(rec(4,1:nnnn),'-*','LineWidth',1.5,'MarkerSize',10);
% ylabel("Cost Function","FontName","Times New Roman","FontSize",24);
% xlabel("Iteration","FontName","Times New Roman","FontSize",24);
% while(1)
%     Gapp=Gapp+gshift;
%     if(nblk>1)
...
%         end
%     end
%     if(BER_cost_d<=op_cost)op_cost=BER_cost_d;gshift=gshift-10;else gshift=gshift+10;shifte=gshift;break;end
% end
return

function BER_cost_d=BER_cost_d_calculation3(RX_block,deltad,ad,deltac,ac,Nt,Nr,block_symbol,Ns,K,L,sc_idx,pilot_symbol,nad,nbd,nac,nbc,SNRdB)
for i=1:Nr
    RX_block(i,:)=interp1((0:length(RX_block(i,:))-1),RX_block(i,:),(0:length(RX_block(i,:))-1)/(1+(ad+deltad)*nad+nbd),'spline');
end
for i=1:Nr
    for n=1:length(RX_block(i,:))
        RX_block(i,n)=RX_block(i,n)*(exp(sqrt(-1)*2*pi*((ac+deltac)*nac+nbc)*(n-1)/9765.625));%39062.5
    end
end
y=RX_block(:,1: Ns: K*Ns);
ola=RX_block(:,1+K*Ns: Ns: K*Ns+(L-1)*Ns);
...
function plotsss(RX_block,deltad,ad,deltac,ac,pilot_bit,Nt,Nr,block_symbol,Ns,K,L,sc_idx,block_bit,pilot_symbol,nad,nbd,nac,nbc)
for i=1:Nr
    RX_block(i,:)=interp1((0:length(RX_block(i,:))-1),RX_block(i,:),(0:length(RX_block(i,:))-1)/(1+(ad+deltad)*nad+nbd),'spline');
end
for i=1:Nr
