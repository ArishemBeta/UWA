function [doppler_scale,cfo]=D2Search(ald,ahd,alc,ahc,P,u,v,sc_idx,Nt,Nr,Ns,K,L,RX_block,pilot_symbol,block_symbol)
recx=zeros(2,300);
d=[ald:0.0005:ahd];
c=[alc:0.25:ahc];
% c=alc;(ahc-alc)/2
...
%         recx(1,i)=ad;
%         recx(2,j)=ac;
        BER_cost=BER_cost_d_calculation(RX_block,0,ad,0,ac,Nt,Nr,block_symbol,Ns,K,L,sc_idx,pilot_symbol);
        recy(i,j)=BER_cost;
...
function BER_cost_d=BER_cost_d_calculation(RX_block,deltad,ad,deltac,ac,Nt,Nr,block_symbol,Ns,K,L,sc_idx,pilot_symbol)
for i=1:Nr
    RX_block(i,:)=interp1((0:length(RX_block(i,:))-1),RX_block(i,:),(0:length(RX_block(i,:))-1)/(1+ad+deltad),'spline');
end
for i=1:Nr
    for n=1:length(RX_block(i,:))
        RX_block(i,n)=RX_block(i,n)*(exp(sqrt(-1)*2*pi*(ac+deltac)*(n-1)/9765.625));%48828.125
    end
end
