...
function [LLR_info LLe_cod] = MAPConvDecoder_R1(LLa_cod,K);
...
A= [-10*ones(Nstate,1) zeros(Nstate,K)]; %initial value for "forward" vector: log(alpha)
A(1,1)= 0;
B= [zeros(Nstate,K) -10*ones(Nstate,1)]; %initial value for "backward" vector: log(beta)
B(1,K+1)= 0;
