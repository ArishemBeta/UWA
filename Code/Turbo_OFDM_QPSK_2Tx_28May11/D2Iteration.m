...
plot(rec(4,1:nnnn),'-*');
ylabel("代价函数","FontName","宋体","FontSize",16);
xlabel("迭代次数","FontName","宋体","FontSize",16);
hold on;
% grad=zeros(2,100);%-0.0015,-0.0010,18,22
% norml=0;normh=100;
...
for i=1:Nr
    for n=1:length(RX_block(i,:))
        RX_block(i,n)=RX_block(i,n)*(exp(sqrt(-1)*2*pi*((ac+deltac)*nac+nbc)*(n-1)/39062.5));%48828.125
    end
end
y=RX_block(:,1: Ns: K*Ns);
ola=RX_block(:,1+K*Ns: Ns: K*Ns+(L-1)*Ns);
