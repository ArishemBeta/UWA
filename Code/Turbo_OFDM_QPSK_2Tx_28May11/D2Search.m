function [doppler_scale,cfo]=D2Search(ald,ahd,alc,ahc,P,u,v,sc_idx,Nt,Nr,Ns,K,L,RX_block,pilot_symbol,block_symbol,pilot_bit,block_bit)
recx=zeros(2,300);
d=[ald:0.0001:ahd];
c=[alc:0.1:ahc];
ld=length(d);
...
for i=1:Nr
    for n=1:length(RX_block(i,:))
        RX_block(i,n)=RX_block(i,n)*(exp(sqrt(-1)*2*pi*(ac+deltac)*(n-1)/39062.5));%48828.125
    end
end
