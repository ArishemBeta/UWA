...
%
%
function [x_Est LLe_cod x_Equ_Soft]= Soft_MIMO_MMSE_LE_packet_qpsk(Y,X,LLa_cod,h,Ndata,K1,K2,L,Nt,Nr,Nbps,SNRdB)
...
Nsblk= 512; %equalize the whole block at once
...
if(0)
    temp= K2+L-1;
    LLa_cod(:,temp*Nbps+1: (temp+Nsblk)*Nbps)= LLe_codA+LLa_cod(:,temp*Nbps+1: (temp+Nsblk)*Nbps); 
end
...
for nd=2: Nrepeat
  ...
  y=Y(:,N-Kp+K2+1:N+K2); %offset K2 zeros 
  if mod(nd-1,pcyc) == 0
      x= X(:,N-Kp+1:N);
  else
      if Nbps == 1
  ...
  if(nd==2)
      h= TimeDomain_MIMO_ChnnEst_fn_new(y, x, Nt, Nr, L, Kp, SNR/1e3); 
  else
      h= TimeDomain_MIMO_ChnnEst_fn(y, x, Nt, Nr, L, Kp, SNR/1e3);
  end
  %PlotChannel(h,Nt,Nr,L); return

  N1= N+1; N2= N+Nsblk+KK-1;
  LLa_cod_blk= LLa_cod(:,(N1-1)*Nbps+1: Nbps*N2);
  if(nd==Nrepeat)
      head=0;
      tail=1;
      [x_EstA LLe_codA]= MIMO_MMSE_LC_TurboEqu_QPSK_fn(Y(:,N+1:N+K1+K2+Nsblk),LLa_cod_blk,h,L,Nt,Nr,Nsblk,Nbps,SNRdB,head,tail);
  end

  x_Est= [x_Est x_EstA]; %collect new equalized symbols
  LLe_cod= [LLe_cod LLe_codA]; %collect LLR for new equalized block
  if(0)
      temp= K2+L-1+(nd-1)*Nsblk;
      LLa_cod(:,temp*Nbps+1: (temp+Nsblk)*Nbps)= LLe_codA+LLa_cod(:,temp*Nbps+1: (temp+Nsblk)*Nbps); 
  end
...
if(0)
    ...
    if(mod(Ndata,Nsblk))
        ...
        h = TimeDomain_MIMO_ChnnEst_fn(y, s, Nt, N_chnn, L, Kp, SNR/1e3); %re-estimated channel
