%Purpose: Process SPACE08 OFDM data: 2Tx,K=4096,16QAM
%Author: Jun Tao
%Place: MST
%Date: June 04, 2011
%
%
function main_mimo_ofdm_2Tx_16qam_4096_EnhChnnEst_batch(factor,L,Chnn_idx,Dist,SNRdB,Niter)
...
Ndata= 45000;           %number of samples in K=4096 segment
if(0)
...
load Data_mod_ofdm_16qam_4096_2Tx.mat  %load Tx symbols
HQAM_TxSym= Data_mod_ofdm_16qam_4096; 
clear Data_mod_ofdm_16qam_4096; 
load Bitstream_ofdm_16qam_4096_2Tx.mat %load Tx bits
if(0)
...
for packet_idx= pkt_idx
    ...
    for nblk= [1:2]
        ...
        Noffset= (1300*2+511+189+1504*24+1300*2+3004)+40000+1300*Nt+511+189+(4+nblk-1)*4576; 
        ...
        for k= 1: Niter
            ...
            for nt= 1: Nt
                ...
                ErrNum(nt,k)= sum(Bitstream_ofdm_16qam_4096(nt,(nblk-1)*Nbit+1: nblk*Nbit)~=Decod_InfoBit);
                ...
                ErrNum1(nt,k)= sum(Bitstream_ofdm_16qam_4096(nt,(nblk-1)*Nbit+1: nblk*Nbit-tblen)~=Decod_InfoBit1);
    ...
    name= ['ErrNum_16QAM_K4096_' packet_name(packet_idx,:)];
...
packet_error_K4096_16QAM= packet_error';
save Results\packet_error_K4096_16QAM packet_error_K4096_16QAM
