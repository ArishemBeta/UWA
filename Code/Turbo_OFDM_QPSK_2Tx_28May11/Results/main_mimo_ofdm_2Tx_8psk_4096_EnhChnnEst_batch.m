%Purpose: Process SPACE08 OFDM data: 2Tx,K=4096,8PSK
%Author: Jun Tao
%Place: MST
%Date: June 04, 2011
%
%
function main_mimo_ofdm_2Tx_8psk_4096_EnhChnnEst_batch(factor,L,Chnn_idx,Dist,SNRdB,Niter)
...
Nrepeat= 2;             %number of OFDM symbols in each packet
...
load Data_mod_ofdm_8psk_4096_2Tx.mat  %load Tx symbols
OPSK_TxSym= Data_mod_ofdm_8psk_4096; 
clear Data_mod_ofdm_8psk_4096; 
load Bitstream_ofdm_8psk_4096_2Tx.mat %load Tx bits
if(0)
...
for packet_idx= pkt_idx
    ...
    for nblk= [1:2]
        ...
        %start of the nblk-th 8PSK block (4)
        Noffset= (1300*2+511+189+1504*24+1300*2+3004)+40000+1300*Nt+511+189+(2+nblk-1)*4576; 
...
%                 FDSym= OPSK_TxSym(:,(nblk-1)*K+1: nblk*K);
                if(1)
                    [S_bar S_var]= PrioriStatCal_8PSK_fn(Nt,K,LLa_cod);
                    Thrshld= 1e-6;
                else
                ...
                FDSym_temp= FDSym;
                for nt= 1: Nt               
                    FDSym_temp(nt,sc_idx_big)= eightpsk(demod_8psk(S_bar(nt,sc_idx_big)));
                end            
                h= FreqDomain_MIMO_ChnnEst_fn_26May11(Y, FDSym_temp, Nt, Nr, Ns, K, L, sc_idx_big, SNR);
            end
                       
            if(~Enh)
                [S_Est S_bar S_var LLe_cod]= MIMO_OFDM_SoftEqu_8psk_fn_28may11(Yblk,LLa_cod,h,K,Ns,L,Nt,Nr,Nbps,SNRdB);
            else
                [S_Est S_bar S_var LLe_cod]= MIMO_OFDM_EnhSoftEqu_8psk_fn_04June11(Yblk,LLa_cod,h,K,Ns,L,Nt,Nr,Nbps,SNRdB);
            end
            ...
            for nt= 1: Nt
                ...
                ErrNum(nt,k)= sum(Bitstream_ofdm_8psk_4096(nt,(nblk-1)*Nbit+1: nblk*Nbit)~=Decod_InfoBit);
                ...
                ErrNum1(nt,k)= sum(Bitstream_ofdm_8psk_4096(nt,(nblk-1)*Nbit+1: nblk*Nbit-tblen)~=Decod_InfoBit1);
    ...
    name= ['ErrNum_8PSK_K4096_' packet_name(packet_idx,:)];
...
packet_error_K4096_8PSK= packet_error';
save Results\packet_error_K4096_8PSK packet_error_K4096_8PSK
