%Purpose: Process SPACE08 OFDM data: 2Tx,K=4096,QPSK
%Author: Jun Tao
%Place: MST
%Date: June 02, 2011
%
%
function main_mimo_ofdm_2Tx_qpsk_4096_EnhChnnEst_batch(factor,L,Chnn_idx,Dist,SNRdB,Niter)
...
load Data_mod_ofdm_qpsk_4096_2Tx.mat  %load Tx symbols
QPSK_TxSym= Data_mod_ofdm_qpsk_4096; 
clear Data_mod_ofdm_qpsk_4096; 
load Bitstream_ofdm_qpsk_4096_2Tx.mat %load Tx bits
if(0)
...
for packet_idx= pkt_idx
    ...
    for nblk= [1: 2]%Nrepeat]

        nblk

        Noffset= (1300*2+511+189+1504*24+1300*2+3004)+40000+1300*Nt+511+189+(nblk-1)*4576; %start of the nblk-th block
        ...
        for k= 1: Niter
            ...
            if(1)
                for nt= 1: Nt
                    ...
                    ErrNum(nt,k)= sum(Bitstream_ofdm_qpsk_4096(nt,(nblk-1)*Nbit+1: nblk*Nbit)~=Decod_InfoBit);
                    ...
                    ErrNum1(nt,k)= sum(Bitstream_ofdm_qpsk_4096(nt,(nblk-1)*Nbit+1: nblk*Nbit-tblen)~=Decod_InfoBit1);
    ...
    name= ['ErrNum_QPSK_K4096_' packet_name(packet_idx,:)];
...
packet_error_K4096_QPSK= packet_error';
save Results\packet_error_K4096_QPSK packet_error_K4096_QPSK
