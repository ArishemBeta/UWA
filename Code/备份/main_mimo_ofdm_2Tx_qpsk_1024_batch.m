...
for packet_idx= [1] %[1 2 3 4 5 6 8 11 14 20 23 26 29 32]
    ...
    for nblk= [1: 8]%Nrepeat]
        ...
        doppler_scale(nblk) = DopplerScaleEstimation(1,1,1,1,1,sc_idx,Nt,Nr,Ns,K,L,RX_data,Chnn_idx,Gap,QPSK_TxSym,Bitstream_ofdm_qpsk_1024,nblk);
...
%             RX_block=RX_data(Chnn_idx,Gap:Gap+K*Ns+120*Ns+Npilot*Ns+Kg*Ns-1);
%             for i=1:Nr
%                 RX_block(i,:)=interp1((0:length(RX_block(i,:))-1),RX_block(i,:),(0:length(RX_block(i,:))-1)*(1+0.005),'spline');
%             end
%             doppler_scale(nblk) = DopplerScaleEstimation(1,1,1,1,1,sc_idx,Nt,Nr,Ns,K,L,RX_data,Chnn_idx,Gap,QPSK_TxSym,Bitstream_ofdm_qpsk_1024,nblk);
%             RX_block=RX_data(Chnn_idx,Gap:Gap+K*Ns+120*Ns-1);
            for i=1:Nr
